...
public class RegisterFrame extends JFrame {
    ...
    public void init() {
        this.setTitle("注册ChatRoom账号");//设置标题
        //System.out.println(DataBuffer.screenSize.width + "\n" + DataBuffer.screenSize.height);
        setBounds((1920 - 387)/2,
                (1080 - 267)/2,
                387, 267);
        ...
        ok.addActionListener(new ActionListener() {
            public void actionPerformed(final ActionEvent e) {
                if (pwdFld.getPassword().length==0 || pwd2Fld.getPassword().length==0) {
                    ...
                    User user = null;
                    try {
                        user = new User(nickname.getText(), new String(pwdFld.getPassword()), sex0.isSelected() ? "m" : "f");
